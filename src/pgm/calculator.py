...
class FreeEnergyCalculation:
    ...
    def calculate(self):
        ...
        volumes, zp_energies, _ = zp(self.NV, self.ratio, calc_input)
...
def zp(NTV, ratio, input: Input):
    ...
    def zp_energy(temperature, frequency, weights):
        ...
        def zp_vib_energy(temperature, frequency):
            kt = K * temperature
        ...
        zp_energies = np.dot(np.nan_to_num(zp_vib_energy(
    ...
    all_volumes = np.array(all_volumes)
    all_zp_energies = np.array(all_energies)
